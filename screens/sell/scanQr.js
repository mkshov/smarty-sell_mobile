...
import ModalScannedProduct from "../create_shipment/components/modalScannedProduct";

const windowWidth = Dimensions.get("window").width;
const windowHeight = Dimensions.get("window").height;
const initialText = "Наведитесь на QR code \n или штрих код";

export default function SellScan({ navigation }) {
    ...
    <SafeAreaProvider>
      ...
      <SafeAreaView style={{ height: windowHeight }} className="pt-11">
        <View style={styles.header}>
          <TouchableOpacity onPress={() => navigation.navigate("sell")} style={styles.backButton}>
            <Icon name="chevron-back" color={"white"} size={25} />
            <Text style={styles.backButton.text}>Назад</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => handleNavigate("sell-cart")}>
