...
import { Alert, Image, Keyboard, KeyboardAvoidingView, Text, TextInput, TouchableOpacity, View } from "react-native";
    ...
    <LinearGradient colors={["#8469A4FF", "#ED83C1FF"]}>
      <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
        <View className=" h-full w-full">
          ...
          <View className="h-full w-[620px] absolute bg-[#FFFFFF0D] rounded-full right-[-110px] top-[15%]"></View>

          {/* title and form */}
          <View className="h-full w-full flex justify-center">
            <View className="flex items-center mb-40">
              <Animated.View entering={FadeInUp.duration(7000).springify()} className="text-white font-bold tracking-wider text-5xl">
                <LogoWhite />
              </Animated.View>
            </View>
            <KeyboardAvoidingView behavior="padding">
              <View className="flex items-center mx-4 space-y-4">
                ...
                <Animated.View entering={FadeInDown.delay(600).duration(1000).springify()} className="w-full">
                  <TouchableOpacity onPress={handleLogin} className="w-full bg-white py-4 rounded-2xl mb-3">
                    <Text className="text-xl font-bold text-[#CD5297] text-center">{isLoading ? "Загрузка..." : "Войти"}</Text>
                  </TouchableOpacity>
                </Animated.View>
              </View>
